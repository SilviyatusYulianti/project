...
if(isset($_POST['send'])){
    ...
    $name = $_POST['number'];
    $name = filter_var($number, FILTER_SANITIZE_STRING);
    $name = $_POST['date'];
    $name = filter_var($date, FILTER_SANITIZE_STRING);
    $name = $_POST['guests'];
    $name = filter_var($guests, FILTER_SANITIZE_STRING);

    $select_contact = $conn->prepare("SELECT * FROM 'contact_form' WHERE name = ? AND number = ? AND guests = ?");
    $select_contact->execute([$name, $number, $date, $guests]);

    if($select_contact->rowCount() > 0){
        $massage[] = 'massage sent already!';
    }else{
        $insert_contact = $conn->prepare("INSERT INTO 'contact_form'(name, number, date, guests) VALUE(?,?,?)");
